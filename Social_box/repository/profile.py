...
def getCurrentProfile(
    db: Session, current_user: schemas.user = Depends(oauth2.get_current_user)
):
    current_userId = (
        db.query(models.User).filter(models.User.email == current_user).first()
    )
    profile = (
        db.query(models.Profile)
        .filter(models.Profile.user_id == current_userId.id)
        .first()
    )
    noOfPost = (
        db.query(models.Post).filter(models.Post.user_id == current_userId.id).count()
    )
    if not profile:
        raise HTTPException(
    ...
    return [
        profile,
        current_userId.first_name,
        current_userId.last_name,
        current_userId.email,
        noOfPost,
    ]
...
def create(
    ...
    new_profile = models.Profile(
        ...
        dob=dob,
        location=location,
    )  # **request.dict() for insert all field at a time
    db.add(new_profile)
    db.commit()
...
def update(
    ...
    if not profile.first() and user.first():
        raise HTTPException(
            ...
            detail=f"Profile/User with id {user_id} is not found",
        )
...
def follow(
    id: str, db: Session, current_user: schemas.user = Depends(oauth2.get_current_user)
):
    curr_user = db.query(models.User).filter(models.User.email == current_user).first()
    follow_filter = (
        db.query(models.Followers)
        .filter(models.Followers.user == id, models.Followers.follower == curr_user.id)
        .first()
    )
    if not follow_filter:
        ...
        curr_followers = (
            db.query(models.Profile).filter(models.Profile.user_id == id).first()
        )
        db.query(models.Profile).filter(models.Profile.user_id == id).update(
            {"followers": curr_followers.followers + 1}
        )
        db.commit()
        curr_following = (
            db.query(models.Profile)
            .filter(models.Profile.user_id == curr_user.id)
            .first()
        )
        db.query(models.Profile).filter(models.Profile.user_id == curr_user.id).update(
            {"following": curr_following.following + 1}
        )
        db.commit()
        return "follow"
    else:
...
def suggestionUser(
    ...
    following = (
        db.query(models.Followers)
        .filter(models.Followers.follower == curr_user.id)
        .values(models.Followers.user)
    )
    ...
    for i in user:
        newSuggestUser = (
            db.query(models.Profile).filter(models.Profile.user_id == i).first()
        )
        newSuggestUser.first_name = (
            db.query(models.User)
            .filter(models.User.id == newSuggestUser.user_id)
            .value(models.User.first_name)
        )
        newSuggestUser.last_name = (
            db.query(models.User)
            .filter(models.User.id == newSuggestUser.user_id)
            .value(models.User.last_name)
        )
        if curr_user.id != newSuggestUser.user_id:
            finalUser = {
                "user_id": newSuggestUser.user_id,
                "profile_pic": newSuggestUser.profile_pic,
                "location": newSuggestUser.location,
                "first_name": newSuggestUser.first_name,
                "last_name": newSuggestUser.last_name,
            }
