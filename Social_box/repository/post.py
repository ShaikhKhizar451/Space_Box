...
def get(
    ...
    for i in posts:
        ...
        user_Picture = (
            db.query(models.Profile).filter(models.Profile.user_id == i.user_id).first()
        )
        likedByCurrUser = (
            db.query(models.PostLikes)
            .filter(
                models.PostLikes.post_id == i.id, models.PostLikes.email == current_user
            )
            .first()
        )
    ...
    if end >= data_length:
        ...
        if page_start > 1:
            response["pagination"][
                "previous"
            ] = f"/getAllPost?page_start={page_start-1}&page_end={page_end}"
        else:
...
def getById(
    ...
    user_Picture = (
        db.query(models.Profile).filter(models.Profile.user_id == post.user_id).first()
    )
    likedByCurrUser = (
        db.query(models.PostLikes)
        .filter(
            models.PostLikes.post_id == post.id, models.PostLikes.email == current_user
        )
        .first()
    )
    if not likedByCurrUser:
...
def update(id: str, request: str, db: Session):
    post = (
        db.query(models.Post).filter(models.Post.id == id).update({"caption": request})
    )
    db.commit()
...
def likePost(
    ...
    like_filter = (
        db.query(models.PostLikes)
        .filter(models.PostLikes.post_id == id, models.PostLikes.email == current_user)
        .first()
    )
    if not like_filter:
        ...
        db.query(models.Post).filter(models.Post.id == id).update(
            {"likes": curr_like.likes + 1}
        )
        db.commit()
        return "liked"
    else:
        del_like = (
            db.query(models.PostLikes).filter(models.PostLikes.post_id == id).delete()
        )
        db.commit()
        curr_like = db.query(models.Post).filter(models.Post.id == id).first()
        db.query(models.Post).filter(models.Post.id == id).update(
